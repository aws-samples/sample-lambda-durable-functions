...
class fraudTransaction {
    constructor(id, amount, location, vendor, score = 0) {
        this.id = id;
        this.amount = amount;
    ...
    async authorize(tx, cusRejection = false) {
    ...
    async sendToFraud(tx, cusRejection = false) {
    ...
    async sendCustomerNotification(callbackId, type, tx) {
...
exports.handler = (0, durable_execution_sdk_js_1.withDurableExecution)(async (event, context) => {
    ...
    tx.score = await context.step("fraudCheck", async () => {
        if (tx.score === 0) {
            ...
            const client = new client_bedrock_agentcore_1.BedrockAgentCoreClient({ region: agentRegion });
            ...
            const command = new client_bedrock_agentcore_1.InvokeAgentRuntimeCommand({
    ...
    if (tx.score < 3)
        return context.step("Authorize", async () => await tx.authorize(tx));
    if (tx.score >= 5)
        return context.step("sendToFraud", async () => await tx.sendToFraud(tx)); //High Risk, Send to Fraud Department
    ...
    if (tx.score > 2 && tx.score < 5) {
        ...
        const verified = await context.parallel("human-verification", [
            ...
            (ctx) => ctx.waitForCallback("SendVerificationEmail", async (callbackId) => {
                await tx.sendCustomerNotification(callbackId, 'email', tx);
            }, { timeout: { days: 1 } }),
            // SMS verification with callback
            (ctx) => ctx.waitForCallback("SendVerificationSMS", async (callbackId) => {
